...
const GadaiPerhiasanPage = () => {
  ...
  const fetchData = async () => {
    ...
    setError(null);
    try {
      const res = await axiosInstance.get('/gadai-perhiasan', { params: { per_page: 1000 } });
              ...
              <TableHead>
                <TableRow>
                  {['No', 'Nama', 'Type', 'Kelengkapan', 'Kode Cap', 'Karat', 'Potongan Batu', 'Berat', 'Nasabah', 'Aksi']
                    .map(headCell => <TableCell key={headCell} align="center">{headCell}</TableCell>)}
                </TableRow>
              </TableHead>
              <TableBody>
                {paginatedData.length > 0 ? (
                  paginatedData.map((row, index) => (
                    <TableRow key={row.id}>
                      ...
                      <TableCell>
                        {Array.isArray(row.kelengkapan)
                          ? row.kelengkapan.join(', ')
                          : row.kelengkapan || '-'}
                      </TableCell>
