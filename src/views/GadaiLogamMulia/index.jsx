...
const GadaiLogamMuliaPage = () => {
  ...
  const fetchData = async (page = 1, perPage = 5) => {
    ...
    setError(null);
    try {
      const res = await axiosInstance.get(`/gadai-logam-mulia?page=${page}&per_page=${perPage}&search=${searchTerm}`);
      if (res.data.success) {
  ...
  return (
    <>
      <Breadcrumb title="Data Gadai Logam Mulia">
        <Typography variant="subtitle2" color="inherit">Home</Typography>
        <Typography variant="subtitle2" color="primary">Daftar Gadai Logam Mulia</Typography>
      </Breadcrumb>
              ...
              <TableContainer component={Paper} sx={{ overflowX: 'auto' }}>
                <Table sx={{ minWidth: 1200 }}>
                  <TableHead>
                    <TableRow>
                      {[
                        'No', 'Nama Barang', 'Type Logam Mulia', 'Kelengkapan',
                    ...
                    {data.length > 0 ? (
                      data.map((row, index) => (
                        <TableRow key={row.id}>
                          ...
                          <TableCell>
                            <IconButton color="primary" onClick={() => navigate(`/edit-gadai-logam-mulia/${row.id}`)}>
                              <EditIcon />
                            </IconButton>
