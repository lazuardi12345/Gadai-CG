...
const DetailGadaiRetroPage = () => {
  ...
  const fetchDetail = async () => {
    ...
    try {
      let url = `/gadai-retro/${id}`;
      if (userRole === "checker") url = `/checker/gadai-retro/${id}`;
      if (userRole === "petugas") url = `/petugas/gadai-retro/${id}`;
      ...
      DOKUMEN_KEYS.forEach((key) => {
        const val = raw.dokumen_pendukung?.[key];
        dokumenPendukung[key] = val ? [val] : [];
      });

      setData({
        ...raw,
        dokumen_pendukung: dokumenPendukung,
        kelengkapan_list: raw.kelengkapan_list || [],
        detail_gadai: raw.detail_gadai || null,
      });
      ...
      <Paper elevation={2} sx={{ position: "sticky", top: 16, zIndex: 20, borderRadius: 3, mb: 3 }}>
        <Box sx={{ display: "flex", alignItems: "center", justifyContent: "space-between", px: 3, py: 2 }}>
          <Stack direction="row" spacing={2} alignItems="center">
            ...
            <Box>
              <Typography variant="h6" fontWeight={700}>Detail Gadai Retro</Typography>
            ...
            <Stack spacing={2} alignItems="center">
              ...
              <Box sx={{ width: '100%' }}>
                ...
                <Stack direction="row" spacing={1} flexWrap="wrap">
                  {d.kelengkapan_list?.length > 0 ? (
                    d.kelengkapan_list.map((k, i) => (
                      <Chip key={i} label={k.nama_kelengkapan} color="success" variant="outlined" size="small" />
                          ...
                          <Grid item xs={6} key={idx}>
                            <Card sx={{ borderRadius: 2, overflow: 'hidden', cursor: 'pointer', boxShadow: 2 }}>
                              <CardActionArea onClick={() => setSelectedImage(url)}>
                                <CardMedia component="img" height="140" image={url} alt={`${key}-${idx}`} />
                              </CardActionArea>
                            </Card>
                          </Grid>
                        ))
...
export default DetailGadaiRetroPage;
